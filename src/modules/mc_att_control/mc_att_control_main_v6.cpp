...
#define MIN_TAKEOFF_THRUST    0.5f
...
void
MulticopterAttitudeControl::poll_subscriptions()
{
	bool updated;
...
MulticopterAttitudeControl::control_attitude(float dt)
{
	...
	// P control, from attitude to attitude rates.
 ...
 */
 ...
 MulticopterAttitudeControl::control_attitude_rates(float dt) 
 {
	...
	math::Vector<3> _rates_err_prev = _rates_sp_prev - _rates_prev;
	/*
	_att_control = rates_p_scaled.emult(rates_err) 
		        + _rates_int 
		        + rates_d_scaled.emult(rates_err - _rates_err_prev) / dt
	...
	// determine whether the integrate work or not by up speed
	// ensure integrate will work after the up speed > 0.1m/s;
	// and when the vehicle lands, takeoff_flag must be false in order to ready for next fly.
